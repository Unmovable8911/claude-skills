...
- **Outline** *(orchestrator reads this)*: `docs/kilians-skills/issues/00-outline.md`. Each line is one slice:
  `<N>. <title> — blocked by: <deps|none> — status: <pending|resolved>`. This is the only doc the orchestrator opens.
- **Latest PRD** *(sub-agent reads)*: newest file in `docs/kilians-skills/prd/` (sort by filename — typically `yyyy-MM-dd-HH:mm.md`). Product context for the implementer.
- **Issue docs** *(sub-agent reads)*: `docs/kilians-skills/issues/<NN>-<title>.md`, where `<NN>` matches the slice number in the outline. The orchestrator only resolves their paths via `ls`, never their contents.
...
Slice: <NN> — <title>
Issue doc: docs/kilians-skills/issues/<NN>-<title>.md   (read it fully first)
Product context: latest PRD in docs/kilians-skills/prd/ (read for background)
